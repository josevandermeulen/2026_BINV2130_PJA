...
Le fichier [`01-ressources/toString_a_copier.txt`](toString_a_copier.txt) contient les méthodes `toString` des classes `Client`, `Pizza` et `PizzaComposable`. Copiez les `toString` dans les bonnes classes.

Les fichiers `01-ressources/Ingredients.java` et `01-ressources/MainPizza.java` sont fournis : copiez-les dans le répertoire `src` de votre projet.

L'interface [`Ingredients`](Ingredients.java) contient uniquement des constantes de type `Ingredient` pouvant être utilisées pour les pizzas.

Exécutez la classe [`MainPizza`](MainPizza.java). Vérifiez que vous avez le bon affichage en comparant avec ce qui est mis dans le fichier `01-ressources/affichage_MainPizza.txt`.

---

*Passez à la [théorie suivante](../../02-seance2/01-ressources/01B_1_theorie.md).*
