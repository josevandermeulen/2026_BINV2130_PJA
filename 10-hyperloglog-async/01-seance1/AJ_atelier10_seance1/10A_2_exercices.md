# Atelier 10 : HyperLogLog appliqué (programmation asynchrone) – séance 1

## Objectif

L'objectif de cette séance est d'appliquer `CompletableFuture` (atelier 9, séance 2) à un traitement concret déjà connu : l'estimation du nombre d'adresses IP uniques avec HyperLogLog (atelier 8, séance 2). Le chargement des fichiers de logs et l'estimation sont lancés de manière asynchrone, puis chaînés sans jamais bloquer.
...
Toutes les classes de l'atelier 8 vous sont fournies **déjà implémentées** ([`HyperLogLog`](src/main/java/domaine/HyperLogLog.java), [`Hasher`](src/main/java/domaine/Hasher.java), [`DefaultHasher`](src/main/java/domaine/DefaultHasher.java), [`Acces`](src/main/java/domaine/Acces.java), [`AnalyseLogs`](src/main/java/main/AnalyseLogs.java), [`AnalyseIp`](src/main/java/main/AnalyseIp.java), la lecture de fichiers) : votre travail porte uniquement sur la nouvelle classe [`AnalyseIpAsync`](src/main/java/main/AnalyseIpAsync.java) (package `main`).

### Consignes

Veuillez créer un nouveau Projet Maven au sein d'IntelliJ nommé `AJ_atelier10_seance1` (voir le [tutoriel Maven](../../../05-mocks/02-seance2/AJ_atelier05_seance2/05B_3_tutoriel-maven.md) si besoin). Récupérez les classes fournies dans `AJ_atelier10_seance1/src/main/java/` (packages `domaine`, `util`, `main`) et les tests fournis dans `AJ_atelier10_seance1/src/test/java/` en conservant l'arborescence. Ajoutez la dépendance JUnit 5 à votre `pom.xml` (voir le `pom.xml` fourni). Copiez également le dossier `logs/` à la racine de votre projet (pas dans un package ni dans `src/`).

Chaque question est vérifiée par les tests JUnit fournis ([`AnalyseIpAsyncTest`](src/test/java/main/AnalyseIpAsyncTest.java)) : exécutez-les directement dans IntelliJ (bouton ▶) pour vérifier votre implémentation au fur et à mesure.
...
*Passez à la [théorie suivante](../../02-seance2/AJ_atelier10_seance2/10B_1_theorie.md).*
