# Atelier 3 : JUnit – séance 1

## Objectif

L'objectif de cet atelier est d'apprendre à écrire des tests automatisés avec JUnit pour vérifier le comportement de classes métier existantes. Cette première séance se concentre sur les tests de la classe [`Prix`](src/domaine/Prix.java) (package `domaine`).
...
9. Tests d'une méthode dont le comportement se ramifie selon un enum ([`TypePromo`](src/domaine/TypePromo.java))
...
- **[`Produit`](src/domaine/Produit.java)** : possède un nom, une marque et un rayon.
...
- **[`ListeProduits`](src/usecase/ListeProduits.java)** (package `usecase`) : garde la liste des produits triés par ordre croissant de leur nom, puis de leur marque et enfin de leur rayon.
...
Cette première séance se limite aux tests de la classe `Prix` ; les classes `Produit` et `ListeProduits` seront testées dans la séance 2.

### Consignes

**Créer le projet :**

Dans IntelliJ, créez un projet intitulé `AJ_atelier03_seance1`. Récupérez les classes `Prix`, `Produit`, `TypePromo`, `ListeProduits`, les exceptions, la classe [`Main`](src/main/Main.java) et l'interface [`Util`](src/util/Util.java) (fournies dans `AJ_atelier03_seance1/`) et mettez-les dans les packages adéquats. Il faudra éventuellement ajouter la SDK. Pour cela :
...
Une promo a un seuil minimal d'achat en-dessous duquel elle n'est pas applicable : demander le prix pour 2 unités sur `prixPub` doit lever une [`QuantiteNonAutoriseeException`](src/exceptions/QuantiteNonAutoriseeException.java) (package `exceptions`).
...
*Passez à la [théorie suivante](../../02-seance2/AJ_atelier03_seance2/03B_1_theorie.md).*
