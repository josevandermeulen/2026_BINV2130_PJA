# Atelier 1 : Rappel orienté objet – séance 1
...
Dans IntelliJ, créez un projet intitulé `AJ_atelier01_seance1`. Vous pouvez mettre vos classes directement dans le répertoire `src` de ce projet.
...
Le fichier [`AJ_atelier01_seance1/toString_a_copier.txt`](toString_a_copier.txt) contient les méthodes `toString` des classes `Client`, `Pizza` et `PizzaComposable`. Copiez les `toString` dans les bonnes classes.

Les fichiers `AJ_atelier01_seance1/Ingredients.java` et `AJ_atelier01_seance1/MainPizza.java` sont fournis : copiez-les dans le répertoire `src` de votre projet.

L'interface [`Ingredients`](src/Ingredients.java) contient uniquement des constantes de type `Ingredient` pouvant être utilisées pour les pizzas.

Exécutez la classe [`MainPizza`](src/MainPizza.java). Vérifiez que vous avez le bon affichage en comparant avec ce qui est mis dans le fichier `AJ_atelier01_seance1/affichage_MainPizza.txt`.

---

*Passez à la [théorie suivante](../../02-seance2/AJ_atelier01_seance2/01B_1_theorie.md).*
