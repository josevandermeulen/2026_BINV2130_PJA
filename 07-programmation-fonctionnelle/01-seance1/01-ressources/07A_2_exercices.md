...
Veuillez créer un nouveau Projet Maven au sein d'IntelliJ nommé `AJ_atelier07_seance1` (voir le [tutoriel Maven](../../../05-mocks/02-seance2/01-ressources/05B_3_tutoriel-maven.md) si besoin). Récupérez les classes fournies dans `01-ressources/src/main/java/` (packages `domaine`, `main`, `lambda`, `code_theorie`) et les tests fournis dans `01-ressources/src/test/java/` en conservant l'arborescence. Ajoutez la dépendance JUnit 5 à votre `pom.xml`.
...
*Passez à la [théorie suivante](../../02-seance2/01-ressources/07B_1_theorie.md).*
