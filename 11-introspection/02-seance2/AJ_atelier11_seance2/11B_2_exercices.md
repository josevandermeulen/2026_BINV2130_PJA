# Atelier 11 : Introspection – séance 2

## Objectif

L'objectif de cette séance est de construire un **mini validateur d'objets par annotations** — le cœur de Jakarta Bean Validation : des annotations posées sur les champs d'une classe (`@NonNul`, `@LongueurMin`) déclarent les contraintes, et un [`Validateur`](src/main/java/be/vinci/validation/Validateur.java) (package `be.vinci.validation`) découvre ces champs par réflexion, lit leur valeur et rapporte chaque contrainte enfreinte. Après la réflexion sur les méthodes en séance 1, c'est la réflexion sur les **champs** : `getDeclaredFields`, `setAccessible`, `Field.get`.
...
Dans cette séance, vous construisez ce mécanisme. La classe [`Etudiant`](src/main/java/be/vinci/domaine/Etudiant.java) (package `be.vinci.domaine`) déclare ses contraintes sur ses champs :
...
et votre `Validateur` les fait respecter : il découvre par réflexion les champs annotés, lit leur valeur (privée !) et produit un [`RapportValidation`](src/main/java/be/vinci/validation/RapportValidation.java) listant chaque [`Violation`](src/main/java/be/vinci/validation/Violation.java).

Cette séance est un projet **indépendant** de la séance 1 — rien à copier, tout le matériel est dans `AJ_atelier11_seance2/`. Les annotations [`LongueurMin`](src/main/java/be/vinci/validation/LongueurMin.java), [`Positif`](src/main/java/be/vinci/validation/Positif.java) et [`Valide`](src/main/java/be/vinci/validation/Valide.java) sont fournies complètes (comme en séance 1, votre travail porte sur leur prise en charge dans le `Validateur`, pas sur leur déclaration) ; seule [`NonNul`](src/main/java/be/vinci/validation/NonNul.java) arrive sans ses méta-annotations. `Violation` et `RapportValidation` (la structure du rapport), la classe [`Adresse`](src/main/java/be/vinci/domaine/Adresse.java) (package `be.vinci.domaine`) et un [`Main`](src/main/java/be/vinci/main/Main.java) (package `be.vinci.main`) de démonstration sont également fournis. Chaque question est vérifiée par les tests JUnit fournis ([`ValidateurTest`](src/test/java/be/vinci/validation/ValidateurTest.java)).

### Consignes

Veuillez créer un nouveau Projet Maven au sein d'IntelliJ nommé `AJ_atelier11_seance2` (voir le [tutoriel Maven](../../../05-mocks/02-seance2/AJ_atelier05_seance2/05B_3_tutoriel-maven.md) si besoin). Récupérez les classes fournies dans `AJ_atelier11_seance2/src/main/java/` (packages `be.vinci.validation`, `be.vinci.domaine`, `be.vinci.main`) et les tests fournis dans `AJ_atelier11_seance2/src/test/java/` en conservant l'arborescence. Ajoutez la dépendance JUnit 5 à votre `pom.xml` (voir le `pom.xml` fourni).
...
Ouvrez l'annotation `NonNul` : elle est déclarée, mais il lui manque ses méta-annotations. Complétez-la pour qu'elle soit conservée à l'exécution et réservée aux **champs** — la cible n'est plus la même qu'en séance 1, voir la théorie ([`11B_1_theorie.md`](11B_1_theorie.md)).
...
*Passez à la [théorie suivante](../../../12-injection-dependances/01-seance1/AJ_atelier12_seance1/12A_1_theorie.md).*
