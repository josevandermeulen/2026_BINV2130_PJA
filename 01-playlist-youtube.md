# BINV2130-1 — Programmation Java : avancé (2026-2027)

Playlist: https://www.youtube.com/playlist?list=PLYYzDB0QZ4xc

## Semaine 1

### Introduction
1. [Introduction au cours de Java avancé](https://www.youtube.com/watch?v=-31P_jow9bs)
2. [GitHub Desktop - cloner et mettre à jour le dépôt du cours](https://www.youtube.com/watch?v=5TTjgaTZnCw)

## Semaine 2

### Interfaces et comparaison d'objets
1. [Interface Comparable en Java](https://www.youtube.com/watch?v=PCjAg07fivY)
2. [Interface Comparator et classe anonyme en Java](https://www.youtube.com/watch?v=gLTAIkKdsK8)

### Types et manipulation d'objets
1. [Clone, copie superficielle et profonde en Java](https://www.youtube.com/watch?v=la-CVGl3k7E)

## Semaine 3

### Tests unitaires avec JUnit
1. [Tests unitaires introduction](https://www.youtube.com/watch?v=UKUbHaxJjRw)
2. [JUnit premiers tests](https://www.youtube.com/watch?v=TkvuBBG-V9o)
3. [Les asserts en JUnit](https://www.youtube.com/watch?v=5swvHf57od8)
4. [Les tests paramétrés en JUnit](https://www.youtube.com/watch?v=fPKqt6oPMdY)
5. [Bonnes pratiques tests unitaires](https://www.youtube.com/watch?v=WwKmx9ghJHU)

## Semaine 4

### Conception de cas de test
1. [Conception de cas de test - cas positifs et négatifs](https://www.youtube.com/watch?v=_3N1241ywmY)
2. [Conception de cas de test - Cas Limites](https://www.youtube.com/watch?v=gZXVC1h3Im0)
3. [Conception de cas de test - Classes équivalences](https://www.youtube.com/watch?v=cd3eu5MWZu0)

## Semaine 5

### Mocks et stubs
1. [Tests unitaires - les stubs](https://www.youtube.com/watch?v=aBwQRyDWl1Y)
2. [Tests unitaires - Mock Objects](https://www.youtube.com/watch?v=d5xIDKCBfyI)
3. [Maven - Mockito](https://www.youtube.com/watch?v=BSIrMuM4UHM)
4. [Tests unitaires - Mockito](https://www.youtube.com/watch?v=oZi6CzLGBY8)

## Semaine 6

### Introduction aux Streams
1. [Les Streams Java - Introduction](https://www.youtube.com/watch?v=q0zMS0_XRSs)
2. [Les Streams - les lambdas expressions en Java](https://www.youtube.com/watch?v=k3kLeB7dd48)

### Programmation fonctionnelle
1. [Introduction à la programmation fonctionnelle](https://www.youtube.com/watch?v=rU_O2mrT8SM)

### Traitement des Streams
1. [Streams - Reduce](https://www.youtube.com/watch?v=LiZTza_KjnI)
2. [Les Streams - Boucles implicites](https://www.youtube.com/watch?v=jZeB6BKyzTM)

### Streams avancés
1. [Streams - Gestion du vide](https://www.youtube.com/watch?v=Oof31l4Raiw)
2. [Les Streams - GroupingBy](https://www.youtube.com/watch?v=AjMDMp0Dw84)

### Pour aller plus loin
1. [Les Streams - debugger les Streams](https://www.youtube.com/watch?v=4hCoZ7HwZsU)
2. [Les Streams un peu de tout](https://www.youtube.com/watch?v=n3P4iniDhOA)

## Semaine 7

### flatMap
1. [Comment aplatir les collections imbriquées en Java avec flatMap](https://www.youtube.com/watch?v=aZHDUchqjyk)

### Design fonctionnel
1. [Immuabilité Java](https://www.youtube.com/watch?v=u0j64SmIoc4)
2. [Mise en pratique du design fonctionnel en Java partie 1 - L'immuabilité](https://www.youtube.com/watch?v=s6Zwdeg3NdI)
3. [Programmation fonctionnelle cas pratique en Java partie 2 - Les exceptions](https://www.youtube.com/watch?v=g0EVQYClhtA)

## Semaine 8

### Lecture de fichiers
1. [Apprendre Java #15 Fichier (File / Reader / Writer / Buffer)](https://www.youtube.com/watch?v=iCobhKfvf3M)

### HyperLogLog
1. [Comment compter des millions de vues ? HyperLogLog](https://www.youtube.com/watch?v=OWBT86qoEqk)

## Semaine 9

### Threads
1. [Les Threads - Bases](https://www.youtube.com/watch?v=4cYaMxE5a58)
2. [Threads - Race Condition](https://www.youtube.com/watch?v=G-BEqoP57Lg)
3. [Threads - Deadlock](https://www.youtube.com/watch?v=SabJvW2oPxQ)

### CompletableFuture
1. [Threads - Completable Future](https://www.youtube.com/watch?v=EE6a93t2c3E)
2. [CompletableFuture - thenCompose vs thenApply](https://www.youtube.com/watch?v=Wz-novO_zlk)
3. [thenCombine](https://www.youtube.com/watch?v=m_AEDUA774Q)
4. [CompletableFuture - Résumé](https://www.youtube.com/watch?v=Pw5OEjXyrSI)

## Semaine 10

### CompletableFuture (rappel)
1. [Threads - Completable Future](https://www.youtube.com/watch?v=EE6a93t2c3E)
2. [CompletableFuture - thenCompose vs thenApply](https://www.youtube.com/watch?v=Wz-novO_zlk)
3. [thenCombine](https://www.youtube.com/watch?v=m_AEDUA774Q)
4. [CompletableFuture - Résumé](https://www.youtube.com/watch?v=Pw5OEjXyrSI)

### HyperLogLog (rappel)
1. [Comment compter des millions de vues ? HyperLogLog](https://www.youtube.com/watch?v=OWBT86qoEqk)

## Semaine 11

### Introspection
1. [Introspection - Machine Virtuelle Java JVM](https://www.youtube.com/watch?v=hozaDXJ7J2Y)
2. [Introspection - typage statique](https://www.youtube.com/watch?v=aFIYXPp-5MY)
3. [Introspection - Inspection d'une classe](https://www.youtube.com/watch?v=s8D-yfzdkSI)
4. [Introspection - Inspection et instanciation d'objets](https://www.youtube.com/watch?v=W7EYb072Rgc)
5. [Introspection - Annotation et mini JUnit](https://www.youtube.com/watch?v=Vo-nmSstLF8)

## Semaine 12

### Factory et injection de dépendances
1. [Interfaces](https://www.youtube.com/watch?v=tLvlrRXwyUk)
2. [Factory method](https://www.youtube.com/watch?v=c7TR0Y_mey8)
3. [Factory Configuration](https://www.youtube.com/watch?v=FdMuh9MQXMI)
4. [Abstract Factory et injection de dépendance via les constructeurs](https://www.youtube.com/watch?v=eZRXt9l4GVM)
