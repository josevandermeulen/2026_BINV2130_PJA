# Atelier 6 : Streams – les bases – séance 2
...
Cet atelier continue la séance 1 : on retrouve les transactions financières effectuées par des traders (classes [`Trader`](src/main/java/domaine/Trader.java) et [`Transaction`](src/main/java/domaine/Transaction.java) (package `domaine`)), cette fois pour agréger, gérer le vide et grouper les résultats.

### Consignes

Veuillez créer un nouveau Projet Maven au sein d'IntelliJ nommé `AJ_atelier06_seance2` (voir le [tutoriel Maven](../../../05-mocks/02-seance2/01-ressources/05B_3_tutoriel-maven.md) si besoin). Récupérez les classes fournies dans `01-ressources/src/main/java/` (packages `domaine`, `main`) et les tests fournis dans `01-ressources/src/test/java/` en conservant l'arborescence. Ajoutez la dépendance JUnit 5 à votre `pom.xml`.

Comme en séance 1, chaque méthode à compléter renvoie une valeur plutôt que de l'afficher : les tests JUnit fournis appellent ces méthodes et vérifient le résultat.
...
Ouvrez la classe [`ExerciceGroupingBy`](src/main/java/main/ExerciceGroupingBy.java) et complétez les méthodes suivantes en utilisant les streams afin de construire et renvoyer les dictionnaires demandés à partir de la liste de transactions présente dans la classe.
...
Complétez les méthodes de la classe [`ExercicesPanaches`](src/main/java/main/ExercicesPanaches.java), qui doivent chacune renvoyer le résultat demandé (pas d'affichage).
...
*Passez à la [théorie suivante](../../../07-programmation-fonctionnelle/01-seance1/01-ressources/07A_1_theorie.md).*
