# Atelier 3 : JUnit – séance 2

## Objectif

L'objectif de cet atelier est de poursuivre l'apprentissage des tests automatisés avec JUnit. Cette deuxième séance se concentre sur les tests des classes [`Produit`](src/domaine/Produit.java) et [`ListeProduits`](src/usecase/ListeProduits.java) (package `usecase`).

## Concepts

1. Tests unitaires (suite) — réemploi des acquis de la séance 1 (`assertAll`, `assertThrows`, `@ValueSource`)
...
Dans IntelliJ, créez un projet intitulé `AJ_atelier03_seance2`. Ajoutez-y les classes de la solution de l'atelier 3 séance 1 (voir `../../01-seance1/02-solution/`), qui contiennent déjà la classe [`PrixTest`](tests/domaine/PrixTest.java). Faites attention aux packages !
...
Demandez un prix pour une date comprise entre deux dates pour lesquelles un prix a été défini. C'est celui de la date antérieure (la plus récente parmi celles passées) qui doit être renvoyé — la logique est similaire à celle des paliers de quantité vus en séance 1, mais appliquée à des dates.
...
*Passez à la [théorie suivante](../../../04-tdd/01-seance1/AJ_atelier04_seance1/04A_1_theorie.md).*
