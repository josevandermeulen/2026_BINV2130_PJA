...
Veuillez créer un nouveau Projet Maven au sein d'IntelliJ nommé `AJ_atelier09_seance1` (voir le [tutoriel Maven](../../../05-mocks/02-seance2/01-ressources/05B_3_tutoriel-maven.md) si besoin). Récupérez les classes fournies dans `01-ressources/src/main/java/` (packages `sync`, `compteur_thread` et `deadlock`) et les tests fournis dans `01-ressources/src/test/java/` en conservant l'arborescence. Ajoutez la dépendance JUnit 5 à votre `pom.xml` (voir le `pom.xml` fourni).
...
Dans le package `sync`, la classe [`Compteur`](src/main/java/sync/Compteur.java) compte de manière synchrone. Exécutez [`TestCompteurSync`](src/main/java/sync/TestCompteurSync.java) et observez le temps d'exécution.
...
**Question 2** :
Faites hériter [`CompteurThread`](src/main/java/compteur_thread/CompteurThread.java) (package `compteur_thread`) de `Thread`, et complétez sa méthode `run`.

**Question 3** :
Complétez [`TestCompteurThread`](src/main/java/compteur_thread/TestCompteurThread.java) : lancez chaque compteur, puis attendez la fin de tous (celui qui finit le plus vite gagne).
...
**Question 5** :
Faites implémenter `Runnable` à [`CompteurRunnable`](src/main/java/compteur_thread/CompteurRunnable.java) — la méthode `count` est déjà fournie.

**Question 6** :
Complétez [`TestCompteurRunnable`](src/main/java/compteur_thread/TestCompteurRunnable.java) : relancez la même course, mais avec des threads construits à partir des `Runnable` (`new Thread(compteur)`), puis attendez leur fin.
...
**Question 9** :
Complétez `count` dans [`CompteurThreadWithRaceCondition`](src/main/java/compteur_thread/CompteurThreadWithRaceCondition.java) : déterminez le compteur gagnant (pause supplémentaire de 10 ms avant l'enregistrement), et affichez-le au sein du compteur.

**Question 10** :
Complétez [`TestCompteurThreadWithRaceCondition`](src/main/java/compteur_thread/TestCompteurThreadWithRaceCondition.java) : lancez les compteurs, attendez leur fin, puis affichez le nom du gagnant. Conservez-le dans un attribut de classe `CompteurThreadWithRaceCondition` — attention, cet attribut peut être lu/modifié par plusieurs threads en même temps.
...
Le package `deadlock` contient une petite application bancaire fournie : des [`Compte`](src/main/java/deadlock/Compte.java), un [`GestionnaireTransferts`](src/main/java/deadlock/GestionnaireTransferts.java) dont la méthode `transferer` synchronise sur les deux comptes concernés, et un [`TestTransfertDeadlock`](src/main/java/deadlock/TestTransfertDeadlock.java) qui lance deux threads faisant chacun 1 000 transferts... en sens inverse l'un de l'autre.
...
*Passez à la [théorie suivante](../../02-seance2/01-ressources/09B_1_theorie.md).*
