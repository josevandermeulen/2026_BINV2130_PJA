...
Ce tutoriel s'applique à la séance 2 de l'atelier 9, section « Appels asynchrones à une API REST ». Il détaille le fonctionnement des méthodes `fetchPosts`, `fetchCommentsForPost` et `fetchUser` de `ApiService`, avant de vous lancer dans les Questions 3 à 7.
