# Atelier 1 : Rappel orienté objet – séance 2
...
Il s'agit de compléter l'application de gestion de commandes de pizzas entamée lors de la séance précédente. Si vous n'avez pas terminé la première séance, vous pouvez récupérer la solution de la séance précédente.

Ci-dessous, se trouve le diagramme de classes complété de l'application. Les classes qui s'y trouvent sont volontairement incomplètes. Dans la classe [`Client`](Client.java), ce qui a été mis en bleu est ce qu'il faut ajouter à celle-ci par rapport à la séance précédente.
...
Dans IntelliJ, créez un projet intitulé `AJ_atelier01_seance2`. Récupérez les classes fournies dans `01-ressources/` (package par défaut) et mettez-les dans le répertoire `src` de votre projet — c'est l'état où la solution de la séance 1 (`../../01-seance1/02-solution/`) vous laisse.
...
Le fichier [`01-ressources/toString_a_copier.txt`](toString_a_copier.txt) contient les méthodes `toString` des classes `LigneDeCommande` et `Commande`. Copiez les `toString` dans les bonnes classes.

La classe [`MenuPizzeria`](MenuPizzeria.java) contient des constantes de type [`PizzaComposee`](PizzaComposee.java) correspondant aux différentes pizzas composées du menu de la pizzeria.

Exécutez la classe [`MainPizzeria`](MainPizzeria.java). Vérifiez que vous avez le bon affichage en comparant avec ce qui est mis dans le fichier `01-ressources/affichage_MainPizzeria.txt`.
...
3. Une liste d'ingrédients vide dans le constructeur de [`Pizza`](Pizza.java) ayant une liste en paramètre ;
4. Un prix inférieur ou égal à 0 ;
5. Une quantité inférieure ou égale à 0.

Vous pouvez utiliser l'interface [`Util`](Util.java) (fournie dans `01-ressources/`) qui offre des méthodes de vérification pour les cas les plus courants.
...
*Passez à la [théorie suivante](../../../02-collections-enumeres/01-seance1/01-ressources/02A_1_theorie.md).*
