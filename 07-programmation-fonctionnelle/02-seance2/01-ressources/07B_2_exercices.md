...
Veuillez créer un nouveau Projet Maven au sein d'IntelliJ nommé `AJ_atelier07_seance2` (voir le [tutoriel Maven](../../../05-mocks/02-seance2/01-ressources/05B_3_tutoriel-maven.md) si besoin). Récupérez les classes fournies dans `01-ressources/src/main/java/` (packages `domaine`, `main`, `vod` — le package `football` sert à la question optionnelle) et les tests fournis dans `01-ressources/src/test/java/` en conservant l'arborescence. Ajoutez la dépendance JUnit 5 à votre `pom.xml`. Résultat attendu de l'exercice flatMap : `01-ressources/affichage_ExercicePortfolio.txt`.

### `flatMap`

La classe `Portfolio` représente un trader avec la liste d'actions qu'il possède : un [`Trader`](src/main/java/domaine/Trader.java) (package `domaine`) et une liste de `String` (les symboles des actions détenues). La classe [`ExercicePortfolio`](src/main/java/main/ExercicePortfolio.java) (package `main`) possède un attribut statique `PRICES`, une `Map` associant chaque symbole à son prix. Le résultat attendu se trouve dans `01-ressources/affichage_ExercicePortfolio.txt`.
...
Le résultat attendu se trouve dans `01-ressources/affichage_ExercicePortfolio.txt`.
...
Les Questions 11 à 16 sont issues de l'examen de septembre 2026 (question « Les streams », 6 points, ± 40 minutes). Utilisez exclusivement l'API Stream (aucune boucle `for`/`while`) et ne modifiez pas la signature des méthodes. Récupérez le package `football` (fourni dans `01-ressources/`) et mettez-le dans vos packages. Vous pouvez vérifier votre implémentation en lançant le `main` de `ExamRunner` (fourni) et en comparant la sortie avec celle donnée ci-dessous.
...
*Passez à la [théorie suivante](../../../08-lecture-fichiers/01-seance1/01-ressources/08A_1_theorie.md).*
