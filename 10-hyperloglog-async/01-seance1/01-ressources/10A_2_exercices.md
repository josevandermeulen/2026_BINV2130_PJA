...
Veuillez créer un nouveau Projet Maven au sein d'IntelliJ nommé `AJ_atelier10_seance1` (voir le [tutoriel Maven](../../../05-mocks/02-seance2/01-ressources/05B_3_tutoriel-maven.md) si besoin). Récupérez les classes fournies dans `01-ressources/src/main/java/` (packages `domaine`, `util`, `main`) et les tests fournis dans `01-ressources/src/test/java/` en conservant l'arborescence. Ajoutez la dépendance JUnit 5 à votre `pom.xml` (voir le `pom.xml` fourni). Copiez également le dossier `logs/` à la racine de votre projet (pas dans un package ni dans `src/`).
...
*Passez à la [théorie suivante](../../02-seance2/01-ressources/10B_1_theorie.md).*
