...
Cette séance est un projet **indépendant** de la séance 1 — rien à copier, tout le matériel est dans `01-ressources/`. Les annotations [`LongueurMin`](src/main/java/be/vinci/validation/LongueurMin.java), [`Positif`](src/main/java/be/vinci/validation/Positif.java) et [`Valide`](src/main/java/be/vinci/validation/Valide.java) sont fournies complètes (comme en séance 1, votre travail porte sur leur prise en charge dans le `Validateur`, pas sur leur déclaration) ; seule [`NonNul`](src/main/java/be/vinci/validation/NonNul.java) arrive sans ses méta-annotations. `Violation` et `RapportValidation` (la structure du rapport), la classe [`Adresse`](src/main/java/be/vinci/domaine/Adresse.java) (package `be.vinci.domaine`) et un [`Main`](src/main/java/be/vinci/main/Main.java) (package `be.vinci.main`) de démonstration sont également fournis. Chaque question est vérifiée par les tests JUnit fournis ([`ValidateurTest`](src/test/java/be/vinci/validation/ValidateurTest.java)).

### Consignes

Veuillez créer un nouveau Projet Maven au sein d'IntelliJ nommé `AJ_atelier11_seance2` (voir le [tutoriel Maven](../../../05-mocks/02-seance2/01-ressources/05B_3_tutoriel-maven.md) si besoin). Récupérez les classes fournies dans `01-ressources/src/main/java/` (packages `be.vinci.validation`, `be.vinci.domaine`, `be.vinci.main`) et les tests fournis dans `01-ressources/src/test/java/` en conservant l'arborescence. Ajoutez la dépendance JUnit 5 à votre `pom.xml` (voir le `pom.xml` fourni).
...
*Passez à la [théorie suivante](../../../12-injection-dependances/01-seance1/01-ressources/12A_1_theorie.md).*
