...
Le fichier [`AJ_atelier01_seance2/toString_a_copier.txt`](toString_a_copier.txt) contient les méthodes `toString` des classes `LigneDeCommande` et `Commande`. Copiez les `toString` dans les bonnes classes.

La classe [`MenuPizzeria`](src/MenuPizzeria.java) contient des constantes de type [`PizzaComposee`](src/PizzaComposee.java) correspondant aux différentes pizzas composées du menu de la pizzeria.

Exécutez la classe [`MainPizzeria`](src/MainPizzeria.java). Vérifiez que vous avez le bon affichage en comparant avec ce qui est mis dans le fichier `AJ_atelier01_seance2/affichage_MainPizzeria.txt`.
...
3. Une liste d'ingrédients vide dans le constructeur de [`Pizza`](src/Pizza.java) ayant une liste en paramètre ;
4. Un prix inférieur ou égal à 0 ;
5. Une quantité inférieure ou égale à 0.

Vous pouvez utiliser l'interface [`Util`](src/Util.java) (fournie dans `AJ_atelier01_seance2/`) qui offre des méthodes de vérification pour les cas les plus courants.
...
*Passez à la [théorie suivante](../../../02-collections-enumeres/01-seance1/AJ_atelier02_seance1/02A_1_theorie.md).*
