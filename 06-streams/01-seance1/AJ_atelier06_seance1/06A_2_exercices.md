# Atelier 6 : Streams – les bases – séance 1
...
Cet atelier traite de streams appliqués à des transactions financières effectuées par des traders (classes [`Trader`](src/main/java/domaine/Trader.java) et [`Transaction`](src/main/java/domaine/Transaction.java) (package `domaine`)).

### Consignes

Veuillez créer un nouveau Projet Maven au sein d'IntelliJ nommé `AJ_atelier06_seance1` (voir le [tutoriel Maven](../../../05-mocks/02-seance2/AJ_atelier05_seance2/05B_3_tutoriel-maven.md) si besoin). Récupérez les classes fournies dans `AJ_atelier06_seance1/src/main/java/` (packages `domaine`, `main`, `code_theorie`) et les tests fournis dans `AJ_atelier06_seance1/src/test/java/` en conservant l'arborescence. Ajoutez la dépendance JUnit 5 à votre `pom.xml`.
...
Ouvrez la classe [`ExercicesDeBase`](src/main/java/main/ExercicesDeBase.java) (package `main`).

Le projet contient les classes `Trader` et `Transaction` décrivant des transactions effectuées par des traders. [`ExercicesDeBaseTest`](src/test/java/main/ExercicesDeBaseTest.java) construit une liste de transactions et teste les méthodes ci-dessous.
...
**Question 4** :
Ouvrez la classe [`ExercicesEmployes`](src/main/java/main/ExercicesEmployes.java). La méthode `listeDesHommes` est à écrire ; en voici la version itérative :
...
*Passez à la [théorie suivante](../../02-seance2/AJ_atelier06_seance2/06B_1_theorie.md).*
